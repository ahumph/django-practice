...
class GeneralTests(TestCase):
    def test_serving_static_files(self):
        # If using static media properly result is not NONE once it finds bassist.jpg
        result = finders.find('images/bassist.jpg')
...
class IndexPageTests(TestCase):
    ...
    def test_index_contains_hello_message(self):
        # Check if there is the message 'bassist Says'
        # Chapter 4
        response = self.client.get(reverse('index'))
        self.assertIn(b'bassist says', response.content)
         
    def test_index_using_template(self):
        # Check the template used to render index page
        # Chapter 4
        response = self.client.get(reverse('index'))
        self.assertTemplateUsed(response, 'bassist/index.html')

    def test_bassist_picture_displayed(self):
        # Check if is there an image called 'bassist.jpg' on the index page
        # Chapter 4
        response = self.client.get(reverse('index'))
        self.assertIn(b'img src="/static/images/bassist.jpg', response.content)
...
class AboutPageTests(TestCase):
    ...
    def test_about_using_template(self):
        ...
        self.assertTemplateUsed(response, 'bassist/about.html')
...
class ModelTests(TestCase):
    ...
    def setUp(self):
        try:
            from populate_bassist import populate
            populate()
        except ImportError:
            print('The module populate_bassist does not exist')
        except NameError:
    ...
    def get_category(self, name):
        ...
        from bassist.models import Category
        try:                  
